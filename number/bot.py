...
def start(message):
    ...
    base = SQLighter(config.database_name)# экземпляр класса SQLighter
    if not base.check_player(chat_id): # проверка на наличие игрока в базе
        base.insert_player(chat_id)# добовление игрока в базу
    base.close()
